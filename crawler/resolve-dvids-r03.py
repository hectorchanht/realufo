#!/usr/bin/env python3
"""Resolve Release 03 DVIDS video IDs → DOD_<record-id> filenames.

Release 03 (6/12/26) video + audio rows in uap-data.csv carry only a
DVIDS Video ID, not a direct media link. The corresponding DOD_*.mp4
lives in the release_03 video bundle (uploaded to R2 at
...
script is dev-only. Modelled verbatim on scripts/resolve-dvids-r01.py;
...
OUT_PATH = os.path.join(REPO, 'scripts', 'dvids2dod-r03.json')
RELEASE_DATE = '6/12/26'
...
def main() -> int:
    ...
    # Gather R03 DVIDS IDs from CSV where PDF|Image Link is empty. Include
